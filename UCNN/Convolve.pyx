...
cpdef conv2d(np.ndarray data, np.ndarray filter1, np.ndarray filter2, int stride=1):
    ...
    while i < (result.shape[0]):
        ...
        while j < (result.shape[1]):
            ...
            temp_number_of_access_to_multiplier, temp_wiT1_table_size, temp_wiT2_table_size, temp_input_table_size = conv_single_stride(
                filter1, filter2,
            ...
            wiT1_table_size += temp_wiT1_table_size
            wiT2_table_size += temp_wiT2_table_size
            input_table_size += temp_input_table_size
...
def convolution(data, kernel, bias, layer_num, stride=1, padding="VALID"):
    ...
    for i in range(0, filter_num, 2):
        ...
        number_of_access_to_accumulator2, number_of_access_to_accumulator3, \
        number_of_access_to_multiplier, wiT1_table_size, wiT2_table_size, input_table_size = conv2d(input_data_padded,
                                                                                                    kernel[:, :, :, i],
                                                                                                    kernel[:, :, :,
                                                                                                    i + 1], stride)
        write_experimental_data_to_excel(worksheet, layer_num, (i, i + 1), number_of_memory_access_weights,
                                         ...
                                         wiT1_table_size, wiT2_table_size, input_table_size)
    return result + bias
